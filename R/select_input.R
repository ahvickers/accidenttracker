...
#' @param x A vector of choices for the input
...
select_input <- function(inputId, label, x, type) {
  if (type == "select") {
    shiny::selectInput(inputId, label, choices = x)
  } else if (type == "multiple") {
    shiny::selectInput(inputId, label, choices = x, multiple = TRUE)
  } else if (type == "checkbox") {
    shiny::checkboxGroupInput(inputId, label, choices = x)
  } else {
